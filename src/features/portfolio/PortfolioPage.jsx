  ...
  return (
    <>
      <div
        ...
        id="portafolio"
      >
        <h1 className="flex container text-white font-bold text-3xl w-full my-12">Portafolio</h1>
        ...
        <div className="container grid gap-4 grid-cols-1 lg:grid-cols-2 grid-rows-2 justify-between w-full h-full mb-24">
          {projects.map((project) => (
            <div
              key={project.id}
              className="flex container justify-between flex-col bg-secondary-color lg:flex flex-grow items-center w-[100%] h-[100%] px-5 py-5 rounded-md sm:mr-5 sm:my-10"
            >
                ...
                <div className="flex">
                  <a href={project.url_live} target="_blank">
                    <button className="flex bg-quinario-color-complement text-white w-full h-full px-4 sm:px-16 rounded-3xl items-center py-1">
                      <FontAwesomeIcon icon={faPlay} className="text-white mr-3 text-3xl" />
                      En vivo
                    </button>
                  </a>
                </div>
